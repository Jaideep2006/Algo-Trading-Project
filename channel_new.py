# -*- coding: utf-8 -*-
"""
Created on Tue Jun 17 10:02:53 2025
...
class Trader:
    def __init__(self):
        self.mid_prices = []
        self.lookback = 20
        self.stop_loss_threshold = 5  # price units (tweak as needed)
        self.trailing_stop_buffer = 3  # trailing stop distance
    ...
    def run(self, state ,position):
        ...
        # === EXIT CONDITIONS ===
            ...
            if mid_price <= self.entry_price - self.stop_loss_threshold:
                orders.append(Order("PRODUCT", price=best_bid, quantity=-position))  # exit long
                self.entry_price = None
            # Trailing stop: update stop if price rises, sell if falls back
            elif mid_price > self.entry_price:
                self.trailing_stop_price = max(self.trailing_stop_price, mid_price - self.trailing_stop_buffer)
            elif mid_price <= self.trailing_stop_price:
                orders.append(Order("PRODUCT", price=best_bid, quantity=-position))
                self.entry_price = None
            # Exit if price crosses above upper band (channel exit)
            elif mid_price > upper_band:
                orders.append(Order("PRODUCT", price=best_bid, quantity=-position))
                self.entry_price = None
            ...
            # Stop-loss
            if mid_price >= self.entry_price + self.stop_loss_threshold:
                orders.append(Order("PRODUCT", price=best_ask, quantity=-position))  # exit short
                self.entry_price = None
            # Trailing stop: update stop if price falls, buy if rebounds
            elif mid_price < self.entry_price:
                self.trailing_stop_price = min(self.trailing_stop_price, mid_price + self.trailing_stop_buffer)
            elif mid_price >= self.trailing_stop_price:
                orders.append(Order("PRODUCT", price=best_ask, quantity=-position))
                self.entry_price = None
            # Exit if price crosses below lower band (channel exit)
